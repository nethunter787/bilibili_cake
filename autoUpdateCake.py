    ...
    for video in os.listdir(folder):
        ...
        try:
            if((time.time() - os.path.getctime(os.path.join(folder,video))) > fileTimeOut) and (video[0:3] not in keyList) and (uid not in uidList): # 删除不想保留的文件
                print(video,'Out Dated File Deleted.')
...
def dfStorage():
    ...
    print("%s 磁盘占用率 %.2f%%, 剩余: %.1fGB"%(timeStr, used/total*100, free / 1073741824))
#===========================主函数===========================

#========================配置=========================

#============ 周期性运行配置===========
run_cnt_max     = 365 # 最多运行多少次后自动停止
updateFreq      = 2 # 0年1月2日3时4分5秒  运行监测程序的频率，每天运行一次
cycleSleepTime  = 30 # 周期性检测时间变化。 单位：秒
VideoTimeOut    = 8*24*60*60 # 单位：秒  Unix时间 
# 如果视频保留了一周的时间没有删除，没有被秒删 说明视频稳定，可在B站下载，没有保留价值。为了节约存储空间 将视频删除。。
...
run_cnt = 0 
while(True):
    newRecordTime = time.localtime(time.time())[updateFreq]  # 2 - 日 3 - 时 4 - 分
    if newRecordTime != lastRecordTime :
        
        # Every Day Task List:
        try:
            cmd = 'python favorcake.py'
            os.system(cmd)
        except Exception as e:
            print(e)
        ...
        run_cnt += 1
        if run_cnt > run_cnt_max - 1:
            print('All Done.')
            break
    lastRecordTime = time.localtime(time.time())[updateFreq]  # 2 - 日 3 - 时 4 - 分
...
# 2023.02.15
# 周期性的删除过期稿件，保留删稿，每天凌晨0点更新稿件状态，更新完成后 删除过期稿件