...
if(uid_up_name == None):
    raise Exception('Up does not exists:%d'%uid_int)
else:
    print(uid_up_name)

video_tid       = 129    # UP视频格式筛选,129为舞蹈视频
...
work_path       = os.getcwd()
AllinOneFolder  = '%s_%s'%(uid_up_name,uid_int) # 空字符 视频会自动存在UP主名字的文件夹中
aimfolder = os.path.join(work_path,AllinOneFolder)
