...
# bp = bypy.ByPy()
# bp.mkdir(remotepath = baidu_dir_name)
# downfilepath_queue  = queue.Queue()
# baidu_upload_thread = threading.Thread(target=thread_baidu_upload, args=('Thread-1',bp,downfilepath_queue))
# baidu_upload_thread.start()

# if(len(sys.argv)==2):
#     downloader_BV(sys.argv[1],Filter=False,Folder_Path=os.getcwd(),Folder='BV_Download')

if(len(sys.argv)==2):
    down_last_hours = int(sys.argv[1])
else:
    down_last_hours = 1

folderpath = os.path.join(work_path,AllinOneFolder)
while(True):
    try:
        # 开始计时
        period_start_time = int(time.time());
        
        # 登陆订阅账号
        cookiejar = cake_login(cookies_filename_9527)
        
        # 默认检测载1小时前的动态
        DynamicVideos = getDynamicVideos(int(time.time())-down_last_hours*60*60)
        time.sleep(4)
        if DynamicVideos == None :
            continue
        else:
            None
        # 换会员账号
        ...
        # 新增动态
        for DynamicVideo in DynamicVideos:
            # if DynamicVideo['bvid'] not in getfolderbvlist(folderpath):
            res_downBV = downloader_BV(DynamicVideo['bvid'],Filter=False,Folder_Path=work_path,Folder=AllinOneFolder)
            if(res_downBV['flag'] == False):
                time.sleep(2)
                print('[×]该视频 %s 重新下载中！！！！'%DynamicVideo['bvid'])
                res_downBV = downloader_BV(DynamicVideo['bvid'],Filter=False,Folder_Path=work_path,Folder=AllinOneFolder)
                
        print('[%s] 动态更新完毕'%time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(time.time()))))
        
        # 控制循环时间在3分钟
        period_end_time = int(time.time());
        if(sleepsec-(period_end_time-period_start_time)>0):
            time.sleep(sleepsec-(period_end_time-period_start_time))
        down_last_hours = 1
    except Exception as e:
        with open('detectcake_error_log.txt','w') as ef:
            ef.write('%s:'%time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(time.time()))))
            ef.write(e.__str__())
            ef.write(traceback.print_exc())
        break
...
# 设置动态获取范围为前1小时
# 2022.07.08
# 添加致谢 修改下载视频的质量为最高 只下载最高
# 2022.07.30
# 文件名下载时添加清晰度说明 替换os.system为os.popen() 自动下载到文件当前目录下文件夹
# 2022.08.06
# 去掉UnVerify选项
# 2022.12.28
# 将cookiejar设置为全局变量,csrf为bili_jct,编写了收藏夹视频转移函数favorcakemove.
# 编写getcakepage函数 解决跨站问题
"""
重要参考内容，致以无比感谢
【Github】 
