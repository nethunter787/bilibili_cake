...
def videoupdate(video_abs_path,rename_flag=False):
    video_root, file_name = os.path.split(video_abs_path)
    vsplit = file_name.split('.')
    video_score = -1
    video_view = 0
    if(len(vsplit)>1):
        video_name = vsplit[-2]
        bvid = video_name.split('_')[-1]
        if(bvid[0:2]=='BV' and vsplit[-1] == 'mp4'): #符合规范
            if video_name[0:3] != '【删】':
                try:
                    video_score,video_view = getBVInfo(bvid)
                    if(video_score == None): # 视频被删除
                        new_file_name = '【删】%s'%file_name
                        lastpath = os.path.join(video_root,file_name)
                        newpath = os.path.join(video_root,new_file_name)
                        os.rename(lastpath,newpath)
                        print('[!]被删：%s'%new_file_name)
                    else: # 视频存在 且获取到了最新分数
                        if(rename_flag):
                            scorenamesplit = file_name.split('】【')
                            new_file_name = '【%02d】【%s' % (video_score,'】【'.join(scorenamesplit[1:]))
                            lastpath = os.path.join(video_root,file_name)
                            newpath = os.path.join(video_root,new_file_name)
                            os.rename(lastpath,newpath) 
                            print('[√]<%02d>%s'%(video_score,new_file_name))
                        else:
                            print('[-]%s'%new_file_name)
                except:
                    print('[×]getBVInfo error: %s'%video_abs_path)
            else:
                print('[-]Skip:%s'%file_name)
        else:
            print('[×]File name error: %s'%video_abs_path)
    
    return (video_score,video_view) # 如果video_score 为None则表示视频被删除
# if(len(sys.argv)>2):
#     rename_flag = True
# else:
#     rename_flag = False
...
# walkpath  = os.path.join(os.getcwd(),updatefolder)
